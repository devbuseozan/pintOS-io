...
thread_tick (void)
{
  ...
  if (thread_mlfqs && current_ticks % TIMER_FREQ == 0)
    {
      mlfqs_update_load_avg_and_recent_cpu ();
      mlfqs_update_all_priorities ();
    }
  else if (thread_mlfqs && current_ticks % TIME_SLICE == 0)
    mlfqs_update_all_priorities ();
               ...
               thread_func *function, void *aux) 
{
  ...
  /* Add to run queue. */
  thread_unblock (t);
  /* Yeni thread daha yüksek priority'liyse yield et. */
...
/* Sleep listesini karşılaştırmak için yardımcı fonksiyon. */
static bool
...
/* Thread'i WAKE_TICK'e kadar uyut. */
void
...
/* Her timer tick'inde çağrılır; uyanması gereken thread'leri uyandırır. */
void
...
/* Priority karşılaştırma: büyük önce. */
bool
...
/* Thread'in efektif priority'sini donations dahil hesapla. */


/* Lock bekleyen thread'den lock sahibine priority donate et (zincirleme). */


/* Bir lock bırakıldığında o lock için yapılan donationları temizle. */


/* Donations sonrası priority'i güncelle. */
static int
...
{
  if (thread_mlfqs)   /* <-- bu satırı ekle */
    return;
  ...
  if (!list_empty (&ready_list))
    {
      struct thread *front = list_entry (list_begin (&ready_list),
                                         struct thread, elem);
...
thread_set_nice (int nice)
{
  ...
  /* Daha yüksek priority'li thread varsa yield et. */
  if (!list_empty (&ready_list))
    {
      struct thread *front = list_entry (list_begin (&ready_list),
                                         struct thread, elem);
...
/* Bir thread'in MLFQS priority'sini hesapla. */
static void
mlfqs_calc_priority (struct thread *t, void *aux UNUSED)
{
  if (t == idle_thread)
    return;
  int p = PRI_MAX
          - FP_TO_INT_NEAR (FP_DIV_INT (t->recent_cpu, 4))
          - t->nice * 2;
...
/* Bir thread'in recent_cpu'sunu güncelle. */
static void
...
/* Her saniye: load_avg ve tüm thread'lerin recent_cpu'sunu güncelle. */
static void
mlfqs_update_load_avg_and_recent_cpu (void)
{
  /* Kaç thread ready veya running (idle hariç)? */
...
/* Her TIME_SLICE tick'te tüm thread'lerin priority'sini güncelle. */
static void
mlfqs_update_all_priorities (void)
{
  thread_foreach (mlfqs_calc_priority, NULL);
  /* ready_list'i yeniden sırala. */
